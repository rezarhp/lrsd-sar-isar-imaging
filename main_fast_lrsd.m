% MAIN_FAST_LRSD
% Fast LRSD-based SAR/ISAR imaging and decomposition demo.
%
...
clc;
clear;  %#ok<CLSCR>
% close all;  % uncomment if you want to close figures

%% Reproducibility (optional)
rng(0); % comment out if you prefer random phase each run

%% Load original scene
Nm = 256;
imgPath = fullfile("images", "ku6.png");

background1 = im2double(rgb2gray(imread(imgPath)));
background_cropped = Extract_ROI(background1, Nm, Nm);

original_scene = background_cropped;
original_scene = original_scene .* exp(1j * rand(size(original_scene)));
...
% Plot initial 2D-FT magnitude in dB (clipped)
...
while ~converged && iter < maxIter
    ...
    % --- LRSD updates ---
    L = svt(F - S + Z1 / delta1, landaB / delta1);
    A = shrink(F - B + Z2 / delta2, landaS / delta1);

    bl = Eh(y) + delta1 * L(:) - Z1(:) - EhE(S);

    % Fast update (your closed-form approximation)
    temp = (1 / delta1) * bl - (1 / (delta1 * (delta1 + 1))) * EhE(bl);
    B = reshape(temp, [M, Ns]);

    bs = Eh(y) + delta2 * A(:) - Z2(:) - EhE(B(:));

    temp1 = (1 / delta2) * bs - (1 / (delta2 * (delta2 + 1))) * EhE(bs);
...
%% Optional: save a quick log
% Ensure results/ exists in your repo
if ~exist("results", "dir"), mkdir("results"); end
save(fullfile("results", "run_summary.mat"), "out", "iter", "elapsedtime", ...
     "Mx", "My", "Sx", "Sy", "Nm", "lambda1", "mu1_min", "rho1", "delta1", "delta2");

% diary('log.txt'); diary off;  % not recommended in a public repo by default


%% ================= Local helper functions =================

function plot_db_image(x, ttl)
% Plot magnitude in dB with clipping (avoids -Inf)
x = abs(x);
